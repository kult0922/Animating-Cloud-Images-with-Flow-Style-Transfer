...
# 生成結果をデモするためのデータセット
def make_demo_dataset(dir, nframes, class_to_idx):
    images = []
    n_video = 0
    n_clip = 0
    for target in sorted(os.listdir(dir)):
...
def make_one_video_dataset(dir, nframes):
    images = []
    item_frames = []
    i = 1
    n_video = 0
    n_clip = 0
    for fi in sorted(os.listdir(dir)):
        if is_image_file(fi):
            file_name = fi
...
class VideoFolder(data.Dataset):
    ...
    def __init__(self, root, nframes,  transform=None, target_transform=None,
                 loader=default_loader, demo=False, test=False):
        classes, class_to_idx = find_classes(root)
        if demo:
            imgs = make_demo_dataset(root, nframes, class_to_idx)
        elif test:
            imgs = make_one_video_dataset(root, nframes)
        else:
